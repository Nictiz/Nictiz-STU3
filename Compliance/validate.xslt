	...
	<xsl:template match="/">
		...
		<compliance>
			<xsl:for-each select="$profiles//profile">
				...
				<xsl:variable name="profile" select="document($filename)/fhir:StructureDefinition"/>
				...
				<profile-compliance>
					...
					<xsl:for-each select="$profile//fhir:mapping/fhir:map">
...
<!-- <dcms>		
		<xsl:for-each select="//objects/object/tag[@name='DCM::Id']">
			<dcm><xsl:attribute name="zib" select="@value"/><xsl:attribute name="name" select="../name"/></dcm>
		</xsl:for-each>
	</dcms> -->
